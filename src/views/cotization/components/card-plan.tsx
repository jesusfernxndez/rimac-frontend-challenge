...
import styles from "./card-plan.module.scss";
import { InsurancePlan } from "@/shared/types";

type CardPlanProps = {
  plan: InsurancePlan;
  discount?: number;
  onSelected: (plan: InsurancePlan) => void;
};

function CardPlan({ plan, discount, onSelected }: CardPlanProps) {
  const priceWithDiscount = discount ? plan.price - discount : plan.price;
  return (
    <div className={styles.card}>
      <div className={styles.card__header}>
        <div className={styles.card__header__container}>
          ...
          {discount && <h5>${plan.price} antes</h5>}
          <h4>${priceWithDiscount} al mes</h4>
        </div>
        <svg
